...
This document describes threats, mitigations, and remaining risks for **Nova Hymnal Web** checkout (Flutterwave) and **Nova Hymnal Backend** premium access.

## Payment flow (intended)

1. User selects **monthly** or **yearly** on the web app (display prices are cosmetic; server enforces amounts).
2. Flutterwave checkout runs in the browser (public key only).
3. On success, the client calls `POST /api/payments/flutterwave/verify` (Next.js), which proxies to Django `POST /api/v1/payments/flutterwave/confirm/`.
4. Django verifies the transaction with **Flutterwave’s API** using `FLUTTERWAVE_SECRET_KEY`, checks amount/currency against `PREMIUM_*` env vars, and writes **`PaymentLedger`** (`transaction_id` unique).
...
| **Amount tampering** (pay less, claim yearly) | Server compares Flutterwave amount to `PREMIUM_MONTHLY_AMOUNT` / `PREMIUM_YEARLY_AMOUNT` (not client `amount`) |
| **Plan mismatch on replay** | Idempotent path rejects `plan_id` ≠ ledger `plan_id` |
| **Secret key exposure** | Flutterwave secret only on Django; removed from Next `.env` |
| **Brute-force confirm** | `django-ratelimit` 20 POST/min per IP on `flutterwave_confirm` |
...
2. **Guest checkout:** Anyone who obtains a valid Flutterwave `transaction_id` can call confirm and activate premium locally until expiry. IDs are hard to guess but not secret; prefer login-before-pay for stricter binding.
...
4. **Manual renewals:** One-time Flutterwave payments do not auto-renew; expiry is time-based only.
...
- [ ] Set Flutterwave + premium env vars on Render
